...
TEMP_FILE="solana_page.html"

# Function to fetch price
fetch_PRICE() {
    echo "Fetching Solana price from CoinMarketCap..."
    curl -s "$SOLANA_URL" -o "$TEMP_FILE"
   ...
   # Extract the price using grep and awk for the updated structure
    ...
    if [ -z "$PRICE" ]; then
        echo "Error: Failed to parse Solana price. Webpage structure might have changed."
        exit 1
    fi
