...
TEMP_FILE="bnb_page.html"

# Function to fetch price
fetch_PRICE() {
    echo "Fetching Bnb price from CoinMarketCap..."
    curl -s "$BNB_URL" -o "$TEMP_FILE"
   ...
   # Extract the price using grep and awk for the updated structure
    ...
    if [ -z "$PRICE" ]; then
        echo "Error: Failed to parse Bnb price. Webpage structure might have changed."
        exit 1
    fi
