...
TEMP_FILE="litecoin_page.html"

# Function to fetch price
fetch_PRICE() {
    echo "Fetching Litecoin price from CoinMarketCap..."
    curl -s "$LITECOIN_URL" -o "$TEMP_FILE"
   ...
   # Extract the price using grep and awk for the updated structure
    ...
    if [ -z "$PRICE" ]; then
        echo "Error: Failed to parse Litecoin price. Webpage structure might have changed."
        exit 1
    fi
