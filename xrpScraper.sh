...
TEMP_FILE="xrp_page.html"

# Function to fetch price
fetch_PRICE() {
    echo "Fetching Xrp price from CoinMarketCap..."
    curl -s "$XRP_URL" -o "$TEMP_FILE"
   ...
   # Extract the price using grep and awk for the updated structure
    ...
    if [ -z "$PRICE" ]; then
        echo "Error: Failed to parse Xrp price. Webpage structure might have changed."
        exit 1
    fi
