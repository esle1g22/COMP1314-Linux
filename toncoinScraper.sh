...
# CoinMarketCap URL
TONCOIN_URL="https://coinmarketcap.com/currencies/toncoin/"

# Temporary file for storing webpage
TEMP_FILE="toncoin_page.html"

# Function to fetch price
fetch_PRICE() {
    echo "Fetching Toncoin price from CoinMarketCap..."
    curl -s "$TONCOIN_URL" -o "$TEMP_FILE"
   ...
   # Extract the price using grep and awk for the updated structure
    ...
    if [ -z "$PRICE" ]; then
        echo "Error: Failed to parse Toncoin price. Webpage structure might have changed."
        exit 1
    fi
