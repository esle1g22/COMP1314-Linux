...
# CoinMarketCap URL
CHAINLINK_URL="https://coinmarketcap.com/currencies/chainlink/"

# Temporary file for storing webpage
TEMP_FILE="chainlink_page.html"

# Function to fetch price
fetch_PRICE() {
    echo "Fetching Chainlink price from CoinMarketCap..."
    curl -s "$CHAINLINK_URL" -o "$TEMP_FILE"
   ...
   # Extract the price using grep and awk for the updated structure
    ...
    if [ -z "$PRICE" ]; then
        echo "Error: Failed to parse Chainlink price. Webpage structure might have changed."
        exit 1
    fi
