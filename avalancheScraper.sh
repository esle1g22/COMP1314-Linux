...
DB_NAME="crypto_db"
TABLE_NAME="avalanche"

# CoinMarketCap URL
AVALANCHE_URL="https://coinmarketcap.com/currencies/avalanche/"

# Temporary file for storing webpage
TEMP_FILE="avalanche_page.html"

# Function to fetch price
fetch_PRICE() {
    echo "Fetching Avalanche price from CoinMarketCap..."
    curl -s "$AVALANCHE_URL" -o "$TEMP_FILE"
   ...
   # Extract the price using grep and awk for the updated structure
    ...
    if [ -z "$PRICE" ]; then
        echo "Error: Failed to parse Avalanche price. Webpage structure might have changed."
        exit 1
    fi
