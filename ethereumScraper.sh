...
TEMP_FILE="ethereum_page.html"

# Function to fetch price
fetch_PRICE() {
    echo "Fetching Ethereum price from CoinMarketCap..."
    curl -s "$ETHEREUM_URL" -o "$TEMP_FILE"
   ...
   # Extract the price using grep and awk for the updated structure
    ...
    if [ -z "$PRICE" ]; then
        echo "Error: Failed to parse Ethereum price. Webpage structure might have changed."
        exit 1
    fi
